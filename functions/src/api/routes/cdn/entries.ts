/**
 * CDA Entries Endpoint
 * GET /cdn/spaces/:space_id/environments/:env_id/entries
 * GET /cdn/spaces/:space_id/environments/:env_id/entries/:entry_id
 * 
...
async function getContentTypeApiId(contentTypeDocId: string): Promise<string> {
  ...
  // Fallback to doc ID if not found
...
/**
 * GET /cdn/spaces/:space_id/environments/:env_id/entries
 * Get all published entries with filtering, search, and pagination
 */
router.get(
  "/",
  authenticate,
  requireAnyScope(["content_delivery.read"]),
  resolveEnvironment,
  async (req: Request, res: Response, next) => {
    try {
      ...
      // If using cursor, validate and parse cursor token
      ...
      // Start with base query for published entries only
      let query = admin.firestore()
        .collection("entries")
        .where("project_id", "==", space_id)
        .where("environment_id", "==", env_id)
        .where("tenant_id", "==", tenantId)
        .where("status", "==", "published");  // CDA only returns published
      ...
      if (parsedQuery.contentType) {
        ...
        if (contentTypeDocId) {
          ...
          // If content type not found, use the passed value directly (might be a doc ID)
          query = query.where("content_type_id", "==", parsedQuery.contentType);
        }
      }
      
      // Apply field filters (Firestore-compatible ones)
      query = applyFilters(
        query,
        parsedQuery.filters,
        parsedQuery.locale,
        undefined // TODO: Pass content type if available
      );
      ...
      // Apply pagination
      if (useCursor) {
        // For cursor pagination, fetch limit + 1 to check if there's a next page
      ...
      if (useCursor && entries.length > parsedQuery.limit) {
        hasMore = true;
        entries = entries.slice(0, parsedQuery.limit);  // Trim to limit
      }
      
      // For backward pagination, reverse results back to correct order
      if (cursorDirection === "backward") {
        entries.reverse();
      }
      
      // Post-process filters that Firestore can't handle
      entries = postProcessFilters(
        entries,
        parsedQuery.filters,
        parsedQuery.locale
      );
      
      // Handle full-text search if query parameter present
      if (parsedQuery.query) {
        entries = await searchEntries(
          parsedQuery.query,
          entries,
          parsedQuery.contentType,
          { spaceId: space_id, envId: env_id, tenantId }
        );
      }
      ...
      if (parsedQuery.include && parsedQuery.include > 0) {
        const linkResult = await resolveLinks(entries, parsedQuery.include, {
          ...
          envId: env_id,
          tenantId,
          onlyPublished: true,  // CDA only resolves published links
      ...
      if (useCursor) {
        // Cursor pagination response (no total, no skip)
        const baseUrl = getBaseUrl(req);
        response = buildCursorResponse(entries, parsedQuery, baseUrl, cursorDirection);
        
        // If we didn't fetch enough results to fill the page, there's no next page
        if (!hasMore && response.pages.next) {
          delete response.pages.next;
        }
      } else {
        // Offset pagination response (traditional)
        response = {
          sys: { type: "Array" },
          total: entries.length,  // TODO: Get real total count without pagination
...
router.get(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      // Parse query for include and locale parameters
      const parsedQuery = parseQueryParams(req.query);
...
function selectFields(entry: any, selectedFields: string[]): any {
  const result: any = {
    sys: entry.sys,  // Always include sys
  };
  ...
  for (const fieldPath of selectedFields) {
    if (fieldPath === "fields") {
      ...
      const fieldName = fieldPath.substring(7); // Remove "fields."
    ...
    } else if (fieldPath.startsWith("sys.")) {
      // Individual sys field (already included, but we could filter sys too)
      // For now, we include all sys fields
 ...
 * TODO: Integrate with Typesense for better performance
 */
async function searchEntries(
  ...
  return entries.filter(entry => {
    // Search in all text and symbol fields
    const searchableText = JSON.stringify(entry.fields).toLowerCase();
    
    // Check if all search terms are present (prefix match)
