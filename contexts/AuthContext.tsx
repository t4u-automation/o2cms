...
// Extended claims interface for our custom claims
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Load claims when user changes
  const loadClaims = async (user: User | null) => {
    if (!user) {
      setClaims(null);
      return;
    }
    try {
      ...
      console.error("[AuthContext] Error loading claims:", error);
      setClaims(null);
    }
  };
  ...
  const refreshToken = async () => {
    try {
      ...
      localStorage.setItem("firebase_token", token);
      // Reload claims after refreshing token
  ...
  return (
    <AuthContext.Provider value={{ user, claims, loading, signInWithGoogle, signInWithGithub, signUpWithEmail, signInWithEmail, logout, refreshToken }}>
      {children}
