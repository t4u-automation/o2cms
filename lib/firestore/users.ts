...
export async function createOrUpdateUser(
  ...
  try {
    ...
    console.log("[O2] User created/updated:", userId, "with role:", user.role);
    return user;
  } catch (error) {
    console.error("[O2] Error creating/updating user:", error);
    throw error;
  }
}
 ...
 */
export async function getTenantUsers(tenantId: string): Promise<O2User[]> {
  try {
    ...
    console.error("[O2] Error fetching tenant users:", error);
    throw error;
  }
}
...
export async function updateUserLastLogin(
  ...
  try {
    ...
    console.log("[O2] User last login updated:", userId);
  } catch (error) {
    console.error("[O2] Error updating user last login:", error);
    throw error;
  }
}
...
export async function removeUserFromTenant(userId: string): Promise<void> {
  try {
    ...
    console.log("[O2] User removed from tenant:", userId);
  } catch (error) {
    console.error("[O2] Error removing user:", error);
    throw error;
  }
}
