...
import { Tenant, O2User, TestCaseStatus } from "@/types";
...
export async function getUserTenant(userId: string): Promise<Tenant | null> {
  try {
    ...
    if (!userDoc.exists()) {
      console.log("[O2] User document not found, needs onboarding");
      return null;
    }
    ...
    if (!userData.tenant_id) {
      console.log("[O2] User has no tenant_id, needs onboarding");
      return null;
    }
    ...
    if (!tenantDoc.exists()) {
      console.error("[O2] Tenant not found for user");
      return null;
    }
    ...
    console.error("[O2] Error fetching user tenant:", error);
    throw error;
  }
}
...
export async function createTenant(
  ...
  try {
    ...
    console.log("[O2] Tenant created:", tenantId);
    return tenant;
  } catch (error) {
    console.error("[O2] Error creating tenant:", error);
    throw error;
  }
}
...
export async function updateTenant(
  ...
  try {
    ...
    console.log("[O2] Tenant updated:", tenantId);
  } catch (error) {
    console.error("[O2] Error updating tenant:", error);
    throw error;
  }
}
...
export async function updateTenantName(
  ...
  try {
    ...
    console.log("[O2] Tenant name updated and setup completed:", tenantId, companyName);
  } catch (error) {
    console.error("[O2] Error updating tenant name:", error);
    throw error;
  }
}
...
async function createDefaultTestCaseStatuses(
  ...
  try {
    ...
    console.log("[O2] Default statuses created for tenant:", tenantId);
  } catch (error) {
    console.error("[O2] Error creating default statuses:", error);
    throw error;
  }
}
